...
export async function getClientCompany(invoiceId, companyNameCache) {
  ...
  for (const c of companies) {
    ...
    const resp = await fetch(`${HUBSPOT_API_BASE}/crm/v3/objects/companies/${c.toObjectId}?properties=name,type,hubspot_owner_id`, {
      headers: { Authorization: `Bearer ${HUBSPOT_SERVICE_KEY}` },
    });
    if (!resp.ok) continue;
    const data = await resp.json();
    const type = (data.properties.type || '').toLowerCase();
    const result = type.includes('client')
      ? { id: c.toObjectId, name: data.properties.name, ownerId: data.properties.hubspot_owner_id || null }
      : null;
