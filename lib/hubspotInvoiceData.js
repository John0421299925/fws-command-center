...
async function fetchParentCompanyIdentity(parentId) {
  try {
    const resp = await fetch(`${HUBSPOT_API_BASE}/crm/v3/objects/companies/${parentId}?properties=name,hubspot_owner_id`, {
      headers: { Authorization: `Bearer ${HUBSPOT_SERVICE_KEY}` },
    });
...
export async function getClientCompany(invoiceId, companyNameCache) {
  ...
  for (const c of companies) {
    ...
    // below.
    const resp = await fetch(`${HUBSPOT_API_BASE}/crm/v3/objects/companies/${c.toObjectId}?properties=name,type,hubspot_owner_id,hs_parent_company_id`, {
      headers: { Authorization: `Bearer ${HUBSPOT_SERVICE_KEY}` },
    });
    if (!resp.ok) continue;
    const data = await resp.json();
